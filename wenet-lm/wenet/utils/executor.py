...
class LMExecutor:
    ...
    def train(self, model, optimizer, scheduler, data_loader, device,
              args):
        ''' Train one epoch
        '''
        model.train()
        clip = args.get('grad_clip', 50.0)
        log_interval = args.get('log_interval', 640)
        num_seen_utts = 0
        num_total_batch = len(data_loader)
        for batch_idx, batch in enumerate(data_loader):
            
            texts = batch
            texts = pad_sequence([torch.from_numpy(xs).int() for xs in texts],
                                True, 0)
            texts = texts.to(device)
            num_utts = texts.size(0)
            if num_utts == 0:
                continue
            context = None
            # Disable gradient synchronizations across DDP processes.
            ...
            num_utts = texts.size(0)
                ...
                # The more details about amp can be found in
                # https://pytorch.org/docs/stable/notes/amp_examples.html
            logpc, logp, count = model(texts)
            accum_grad = 1
            loss = logpc.mean() / accum_grad
            if batch_idx%100 == 0:
                print("iter: ", batch_idx, "Loss: ", float(loss))
            loss.backward()
            num_seen_utts += num_utts
            if batch_idx % accum_grad == 0:
                grad_norm = clip_grad_norm_(model.parameters(), clip)
            ...
            if batch_idx % log_interval == 0:
                ...
                log_str = 'TRAIN Batch {}/{} loss {:.6f} '.format(
                    ...
                    loss.item() * accum_grad)
                if loss is not None:
                    log_str += 'loss {:.6f} '.format(loss.item())
                logging.debug(log_str)
