...
node_rank=0
# data
data=//ceph/sz_ts80_new/edlonwei/wenet-lm/wenet/examples/aishell/s0/data_aishell
...
if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
    ...
    tools/text2token.py -s 1 -n 1 data/train/text | cut -f 2- -d" " \
        > ${lmdatadir}/train.txt
    tools/text2token.py -s 1 -n 1 data/dev/text | cut -f 2- -d" " \
        > ${lmdatadir}/valid.txt

    python wenet/bin/lm_train.py \
        --config ${lm_config} \
        --ngpu 1 \
        --verbose 1 \
        --outdir ${lmexpdir} \
